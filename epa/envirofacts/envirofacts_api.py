...
class Envirofacts(API):
    ...
    def __init__(self):
        ...
        self.output_formats = ['XML', 'JSON', 'CSV', 'EXCEL']
        self.default_output_format = 'xml'
    ...
    def call_api(self, table_lst, column, value, **kwargs):
        ...
        try:
            op = kwargs.pop('operation').upper()
        except KeyError:
            op = self.default_operation
        if op not in self.operations:
            raise ValueError("illegal operation: {}".format(op))
        url_list = [self.base_url, table_lst[0], column, op, quote(value)]
